...
/* 세마포어에 대한 업(V) 연산. SEMA의 값을 증가시키고, 대기 중인 스레드가 있으면
 * 하나를 깨움. */
...
/* LOCK 획득. 필요 시 사용 가능해질 때까지 대기. 현재 스레드가 이미 락을 보유
 * 중이면 안 됨. */
void lock_acquire(struct lock *lock);

/* LOCK 획득 시도. 성공 시 true, 실패 시 false 반환. 현재 스레드가 이미 락을
 * 보유 중이면 안 됨. */
...
/* LOCK을 원자적으로 해제하고 COND에 시그널이 올 때까지 대기. 시그널 수신 후
 * 반환 전 LOCK 재획득. */
void cond_wait(struct condition *cond, struct lock *lock);

/* COND(LOCK으로 보호)에서 대기 중인 스레드 하나에 깨우라는 시그널 전송. 호출 전
 * LOCK 보유 필요. */
...
bool compare_thread_priority_cond(const struct list_elem *a, const struct list_elem *b, void *aux);
/* 최적화 장벽.
